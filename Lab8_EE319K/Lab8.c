...
uint32_t Convert(uint32_t data){
   return ((162 * (data)) / (4096 + 24) ); // Formula from Calibration.xls sheet --> from lab lecture
}
...
void SysTick_Handler(void){ // every 100 ms
  PF1 ^= 0x02;     // Heartbeat // Toggle LED for Texas_Display
  // write this
	MailValue = ADC_In();	// MailValue is a global variable
	MailStatus = 1;				// Mail status is a clobal variable for main
}
...
int main(void){  
 ...
 // 32-point averaging  
  PortF_Init();
	SysTick_Init(8000000);	// 80Mhz - 12.5ns // 10 Hz = 100ms --> interrupt every 100ms ; every bus cycle is 12.5ns // How many bus cycles go into 100ms 
  // 100ms / 12.5ns --> period for systick // SysTick is simply a down counter--> based on a frequency
	// other initialization, like mailbox
  EnableInterrupts();
  while(1){
    // wait on mailbox
		while(MailStatus == 0) {}; // Wait on mailbox
		PF1 ^= 0x08;       // Heartbeat
    // write this 
